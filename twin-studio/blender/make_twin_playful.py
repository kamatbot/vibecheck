...
material('Skin','c68d6a',.62,subsurface=.045)
material('Lips','aa665b',.62,subsurface=.025)
material('Hair','49325f',.65)
material('Brows','49325f',.7)
material('Iris','6b4b38',.4)
material('Sclera','f4ece0',.38)
material('Top','a496ec',.86)
material('Pants','34415d',.88)
material('Shoes','ff977d',.62)
...
material('Iris_Rim','402d29',.42)
material('Iris_Inner','916643',.4)
material('Iris_Fiber','795640',.42)
material('Top_Seam','9384d4',.87)
material('Pants_Seam','2c3850',.88)
material('Shoe_Panel','de806b',.74)
...
# A compact, softly tapered avatar face. The lower face is shorter and cheeks
# broader than the first sculpt; simple continuous volumes carry the expression.
PROFILE=[(1.414,.008,.018,.020),(1.420,.036,.057,.039),
 (1.433,.070,.079,.055),(1.454,.094,.088,.077),(1.480,.106,.098,.094),
 (1.512,.116,.101,.105),(1.545,.115,.098,.109),(1.577,.110,.094,.109),
 (1.615,.108,.093,.104),(1.650,.098,.083,.092),(1.682,.066,.055,.061),
...
def face_y(x,z):
    ...
    d=0
    # Shallow sockets and lifted, full cheeks avoid a hollow mask appearance.
    for side in (-1,1):
        ex=side*.046
        d += .0030*gauss(x,z,ex,1.566,.033,.022)
        d -= .0085*gauss(x,z,side*.063,1.516,.040,.029)
        d -= .0030*gauss(x,z,side*.041,1.607,.036,.014)
        d -= .0045*gauss(x,z,side*.013,1.532,.009,.007)
    d -= .010*gauss(x,z,0,1.554,.011,.024)
    d -= .020*gauss(x,z,0,1.540,.015,.012)
    d -= .003*gauss(x,z,0,1.501,.013,.011)
    d -= .004*gauss(x,z,0,1.485,.043,.021)
    d -= .004*gauss(x,z,0,1.436,.037,.018)
    d += .0006*gauss(x,z,0,1.503,.0035,.008)
    return base+d
...
# Rounded eye openings follow the sculpt with shallow convex lenses. This
# prevents protruding eyeballs and heavy shadowed rims while keeping real lids.
EYE_Z=1.568;EYE_X=.046;EYE_Y=-.075;EYE_R=.038
EYE_W=.0285;EYE_UP=.0170;EYE_DOWN=.0145
def eyefront(dx,dz,side):
    fullness=max(0,1-(dx/.032)**2-(dz/.021)**2)
    return face_y(side*EYE_X+dx,EYE_Z+dz)-.0016-.0032*fullness
def eyelid_point(side,t,outer=0):
    dx=cos(t)*(EYE_W+outer)
    h=(EYE_UP if sin(t)>0 else EYE_DOWN)+outer*.75
    dz=sin(t)*h+side*dx*.025
    return (side*EYE_X+dx,eyefront(dx,dz,side),EYE_Z+dz)

for side in (-1,1):
    sn='L' if side<0 else 'R';part='eye_'+sn
    ev=[(side*EYE_X,eyefront(0,0,side),EYE_Z)]
    EF=[];rings=9;seg=64
    for j in range(1,rings+1):
        ...
        for i in range(seg):
            ...
            ev.append((side*EYE_X+dx,eyefront(dx,dz,side),EYE_Z+dz))
    ...
    # Broad smooth transitions blend into skin without a thick raised eyelid.
    lv=[];lf=[]
    for k,off in enumerate((0,.0010,.0028,.0054)):
        for i in range(seg):
            p=list(eyelid_point(side,2*pi*i/seg,off))
            base=face_y(p[0],p[2])
            p[1]=p[1]-.00020 if k==0 else base-(.0025,.0012,.00025)[k-1]
            lv.append(tuple(p))
    for k in range(3):
        for i in range(seg):
            a=k*seg+i;b=k*seg+(i+1)%seg;lf.append((a,b,b+seg,a+seg))
    mesh('Eyelids_'+sn,lv,[tuple(reversed(f)) for f in lf],'Skin',part)
    # One restrained upper line gives a readable, friendly animated-film eye.
    pts=[eyelid_point(side,pi*t/12,.0002) for t in range(13)]
    pts=[(x,y-.00040,z+.00015) for x,y,z in pts]
    tube('Lash_Line_'+sn,pts,.00075,'Brows',part,radii=[.08,.65,.9,1,.8,.5,.12],resolution=2,sides=6)
    # Lens rings sit flush on the eye globe; pupil and dark limbal ring inset.
    iv=[];ifs=[];rad=[0,.0083,.0090,.0125,.0147,.0156]
    for rr in rad:
        for i in range(64):
            t=2*pi*i/64;dx=rr*cos(t);dz=rr*sin(t)+.0010
            almond=sqrt(max(.01,1-(dx/EYE_W)**2))
            dz=clamp(dz,-EYE_DOWN*almond+side*dx*.025+.00025,EYE_UP*almond+side*dx*.025-.00025)
            iv.append((side*EYE_X+dx,eyefront(dx,dz,side)-.00030,EYE_Z+dz))
    ...
    for p in io.data.polygons:
        ring=p.index//64
        p.material_index=1 if ring==0 else 2 if ring==1 else 3 if ring==4 else (4 if p.index%23==0 else 0)
    for j,(dx,dz,r) in enumerate([(-.0050,.0065,.0030),(.0054,-.0045,.0011)]):
        ellipsoid('Eye_Glint_'+sn+'_'+str(j),(side*EYE_X+dx,eyefront(dx,dz,side)-.00070,EYE_Z+dz),(r,.00045,r),'Catchlight',part,20,12)
    # Soft lifted brows, clean enough to read at full-body viewing distance.
    pts=[]
    for i in range(11):
        t=i/10;xx=side*(.017+.064*t);zz=1.609+.006*sin(pi*t)-.004*t
        pts.append((xx,face_y(xx,zz)-.0024,zz))
    tube('Brow_'+sn,pts,.0034,'Brows','brow_'+sn,radii=[.5,.9,1,.9,.60,.10],resolution=3,sides=8,flat=.55)

# Small nostril accents under the single soft upturned nose volume.
for side in (-1,1):
    xx=side*.0097;zz=1.5300;yy=face_y(xx,zz)
    ellipsoid('Nostril_'+str(side),(xx,yy-.0004,zz),(.0032,.00070,.0017),'Nostril','nose',24,12)

# A broad closed smile is the neutral geometry, with softly simplified lips.
MOUTH_Z=1.482;MOUTH_W=.043
def mouthline(x):return MOUTH_Z+.0090*(abs(x)/MOUTH_W)**2
for upper in (True,False):
    vv=[];ff=[];U=64;V=9
    for j in range(V):
        t=j/(V-1)
        for i in range(U):
            x=-MOUTH_W+2*MOUTH_W*i/(U-1);f=max(0,1-(x/MOUTH_W)**2)**.7
            cupid=1+.12*exp(-((abs(x)-.011)/.0055)**2)-.10*exp(-(x/.005)**2)
            height=(.0055*cupid if upper else .0075)*f
            z=mouthline(x)+(height*t if upper else -height*t)
            y=face_y(x,z)-.0004-.0030*f*sin(pi*t*.88)-.0016*f*(1-t)
            vv.append((x,y,z))
    for j in range(V-1):
        for i in range(U-1):
...
pts=[]
for i in range(17):
    x=-MOUTH_W+.0003+(2*MOUTH_W-.0006)*i/16;z=mouthline(x);pts.append((x,face_y(x,z)-.0024,z))
